...
+ Построение выполняется рекурсивным спуском от корня дерева

+ Рекурсивынй метод должен содеражтьь ссылку на исходные данные значения, например, в виде итераторов

+ Если текущий узел является листом, то надо занести в него значение

+ Если текузий узел не является листом, то надо вызвать метод для левого и правого узла, в после найти значение, которое будет храниться в текущем узле

### Нерекурсивынй метод

+ Исходные занчения записываем в массив последовательно со смещением $2^K$ ОТ начала

+ Если исходных значений меньше чем элементов, то оставшиеся заполняем `identity_element`

+ Для всех элементов массива от $2^k-1 до $1$ включительно записываем результат применения заданной операции к дочерним узлам `data[i]=data[2*i] op data[2*i+1]`

+ Для возведения числа 2 в некоторую степень $m$ следуюет использовать двоичную операцию сдвига `1<<m`

### Операции над деревом | Изменение одного значения

Для изменения одного значения в дереве отрезков нужно выполнить следующие шаги:

1. Рекурсивно спуститься от корня дерева до соответствующего листового узла, содержащего изменяемое значение. Для этого используется бинарный поиск по диапазону индексов каждого узла.

2. Обновить значение листового узла, соответствующего изменяемому индексу, в соответствии с требуемым изменением.

3. Подняться обратно к корню дерева, обновляя значения узлов на каждом уровне. Для каждого внутреннего узла, значение обновляется путем агрегации значений его дочерних узлов.

```cpp
// Рекурсивная функция для изменения значения в дереве отрезков
void updateValue(SegmentTreeNode* node, int index, int value) {
    // Проверка достижения листового узла
    if (node->start == node->end) {
        node->value = value;
        return;
    }

    // Рекурсивный спуск к соответствующему дочернему узлу
    int mid = (node->start + node->end) / 2;
    if (index <= mid) {
        updateValue(node->left, index, value);
    } else {
        updateValue(node->right, index, value);
    }

    // Обновление значения текущего узла
    node->value = node->left->value + node->right->value; // Пример: сумма значений
}

```

В данном примере функция `updateValue` принимает указатель на корневой узел дерева, индекс элемента для изменения и новое значение. Алгоритм рекурсивно спускается к соответствующему листовому узлу и обновляет его значение. Затем значения на каждом уровне поднимаются обратно к корню, обновляя значения внутренних узлов путем агрегации значений их дочерних узлов.

### Функция `accumulate`

Функция `accumulate` на интервале в дереве отрезков позволяет вычислить агрегацию значений на заданном интервале. Эта функция возвращает результат агрегации значений элементов, содержащихся в заданном интервале.

Для выполнения операции ё на интервале в дереве отрезков нужно следовать следующим шагам:

1. Проверить, перекрывает ли текущий узел дерева отрезков интервал полностью, частично или не перекрывает его вовсе. Для этого сравнить границы интервала и границы текущего узла дерева.

2. Если текущий узел полностью содержится внутри заданного интервала, вернуть его значение как результат.

3. Если текущий узел не перекрывает заданный интервал, вернуть значение-идентификатор, которое указывает на нейтральный элемент операции агрегации. Например, для операции суммирования это может быть 0, а для операции нахождения минимума - бесконечность.

4. Если текущий узел перекрывает заданный интервал частично, рекурсивно вызвать `accumulate` для его дочерних узлов, которые перекрывают заданный интервал. Объединить результаты, применив операцию агрегации.
...
T seg_tree::accumulate(int l, int r) {
    T result = identity_element;  // Инициализация результата нейтральным элементом операции

    // Подъем по дереву отрезков, начиная с левой границы l и заканчивая правой границей r
    for (l += 1 << k; l < r; l /= 2, r /= 2) {
        if (l & 1) {
            result = op(result, data[l++]);  // Если l - нечетное, применяем операцию к data[l] и увеличиваем l на 1
        }
        if (r & 1) {
            result = op(result, data[--r]);  // Если r - нечетное, применяем операцию к data[r-1] и уменьшаем r на 1
        }
    }
...
```
+ `T result = identity_element;`: Инициализируем переменную `result` нейтральным элементом операции агрегации. Это значение будет постепенно обновляться по мере применения операции агрегации к элементам интервала.
+ `for (l += 1 << k; l < r; l /= 2, r /= 2)`: Инициализируем цикл для подъема по дереву отрезков от левой границы l до правой границы `r`. Здесь `k` - глубина дерева отрезков.
+ `if (l & 1) result = op(result, data[l++]);`: Если `l` - нечетное число, значит оно является правым потомком своего родителя, поэтому применяем операцию агрегации к `data[l]` и увеличиваем `l` на 1.
+ `(r & 1) resul = op(result, data[--r]);`: Если `r` - нечетное число, значит оно является правым потомком своего родителя, поэтому применяем операцию агрегации к `data[r-1]` и уменьшаем `r` на 1.
+ `return result;`: Возвращаем результат агрегации значений на заданном интервале.

### Использование дерева отрезков

+ Запросы на отрезке 

1. Д`ерево сумм, минимумов, максимумов и других функций` позволяет быстро отвечать на запросы связанные с поиском значения на некотором отрезке

Для неизменяемых последовательностей существуют более простые способы решения, однако, дерево отрезков позволяет редактировать значения элементов

Дерево отрезков не позволяет легко выполнить вставку и удаление элементов в произвольном месте последовательности, но можно попробовать реализовать метод вставки элементов в конец дерева



2. Метод `first_apper` (нерекурсивен)

Позволяет найти самое левое значение не меньше, чем $х$ 
Для реализации требуется построить дерево максимумов 

Поиск идет от корня. Если корень в дереве $<х$, то искомых значений нет. Иначе, если элемент в левом дочернем узле $<х$, то идет к правому узлу, иначе – к левому. Если дошли до листа, то ответ 

3. `K-тая порядковая статистика в массиве` 

Есть массив из n элементов. В любой момент элемент можно удалить из массива или вернуть его на свое место.
Требуется отвечать на запросы, связанные с поиском к-го по порядку элемента из существующих в массиве. 

Строим дерево сумм, где листы являются `0` или `1` в зависимости от присутствия соответствующего элемента в массиве. 
При удалении или возвращении элемента в массив пересчитываем дерево сумм.
Для поиска `к-той` порядковой статистики выполняем поиск от корня дерева методом похожим на `first_upper`
